...
!
      ...
      do k=kb,ke
        do j = jb,je
          do i=2,nx1
            dena=iflag*0.5_CUSTOM_REAL*(den(i,j,k)+deno(i,j,k))&
                +(1._CUSTOM_REAL-dble(iflag))*den(i,j,k)
            a=1.0_CUSTOM_REAL/dena
...
!           Nonuniform mesh
            ...
            dpedx(i,j,k)=(  (pe(i+1,j-1,k  )+2.*pe(i+1,j,k  )+pe(i+1,j+1,k  ))/4._CUSTOM_REAL    &
                          - (pe(i-1,j-1,k  )+2.*pe(i-1,j,k  )+pe(i-1,j+1,k  ))/4._CUSTOM_REAL    &
                         )*dxa
            dpedy(i,j,k)=(  (pe(i-1,j+1,k  )+2.*pe(i,j+1,k  )+pe(i+1,j+1,k  ))/4._CUSTOM_REAL    &
                          - (pe(i-1,j-1,k  )+2.*pe(i,j-1,k  )+pe(i+1,j-1,k  ))/4._CUSTOM_REAL    &
                         )*dya
            dpedz(i,j,k)=0.0_CUSTOM_REAL
          enddo
        enddo
      enddo
...
!
      ...
      real(kind=CUSTOM_REAL) :: teny,btot,curr_tot,tjdotb,dexdy,dexdz,deydx,deydz
...
!          Nonuniform mesh
            dxa=a/(2._CUSTOM_REAL*meshX%dxc(i))
            dya=a/(2._CUSTOM_REAL*meshY%dxc(j+1))                  ! integer index in y direction starts at 0
            dza=a/(2._CUSTOM_REAL*meshZ%dxc(k+1))                  ! integer index in z direction starts at 0
 

            dbxdy=+bx(i  ,j+1,k  )+bx(i+1,j+1,k  )&
                  -bx(i  ,j  ,k  )-bx(i+1,j  ,k  )
            dbxdz= 0.0_CUSTOM_REAL
            dbydx=+by(i+1,j  ,k  )+by(i+1,j+1,k  )&
                  -by(i  ,j  ,k  )-by(i  ,j+1,k  )
            dbydz= 0.0_CUSTOM_REAL
            ...
            bxav=0.25_CUSTOM_REAL*(bx1+bx2+bx3+bx4)
            byav=0.25_CUSTOM_REAL*(by1+by2+by3+by4)
            bzav=0.25_CUSTOM_REAL*(bz1+bz2+bz3+bz4)
!
...
!
		    ...
		    byy = byav
		    bzz = bzav
		    btot = sqrt(bxx**2_CUSTOM_REAL + byy**2_CUSTOM_REAL + bzz**2_CUSTOM_REAL)
                    xtmp1m = sqrt(xj**2_CUSTOM_REAL + yj**2_CUSTOM_REAL + zj**2_CUSTOM_REAL)
                    xtmp2m = 1.E-12_CUSTOM_FLUX	!LAURAXX
		       ...
		       tenz = tjdotb*bzz/btot
                       ...
                       xtmp1m = sqrt(xj**2_CUSTOM_REAL + yj**2_CUSTOM_REAL + zj**2_CUSTOM_REAL)
                       xtmp2m = 1.E-12_CUSTOM_FLUX	!LAURA
		       curr_tot = max(xtmp2m,xtmp1m)  !!!TYPO was xmtp1m
...
!
      ...
      call XREALBCC_PACK_E_2D(EX,EY,EZ,one,NX,NY,NZ)
...
!          Nonuniform mesh
            curlex(i,j,k)=dezdy/(2._CUSTOM_REAL*meshY%dxn(j+1))-deydz/(2._CUSTOM_REAL*meshZ%dxn(k+1))        ! integer index in y and z directions start  at 0
            curley(i,j,k)=dexdz/(2._CUSTOM_REAL*meshZ%dxn(k+1))-dezdx/(2._CUSTOM_REAL*meshX%dxn(i  ))        ! integer index in z       direction  starts at 0
            curlez(i,j,k)=deydx/(2._CUSTOM_REAL*meshX%dxn(i  ))-dexdy/(2._CUSTOM_REAL*meshY%dxn(j+1))        ! integer index in y       direction  starts at 0
...
!********************
         ...
         tempx1=tempx1+2._CUSTOM_REAL*curlex
         tempy1=tempy1+2._CUSTOM_REAL*curley
         tempz1=tempz1+2._CUSTOM_REAL*curlez
...
!************************
  ...
  10     continue
         ...
         CALL XREALBCC_PACK_B_2D(BX,BY,BZ,one,NX,NY,NZ)
...
!
      ...
      real(KIND=CUSTOM_REAL) :: BX1, BX2, BX3, BX4, BY1, BY2, BY3, BY4  !LAURA
      real(KIND=CUSTOM_REAL) :: BZ1, BZ2, BZ3, BZ4, BXAV, BYAV, BZAV  !LAURA
      one=1  !LAURA
...
!          Nonuniform mesh
            curlbx_scalar=dbzdy/(2._CUSTOM_REAL*meshY%dxc(j+1))-dbydz/(2._CUSTOM_REAL*meshZ%dxc(k+1))
            curlby_scalar=dbxdz/(2._CUSTOM_REAL*meshZ%dxc(k+1))-dbzdx/(2._CUSTOM_REAL*meshX%dxc(i  ))
            curlbz_scalar=dbydx/(2._CUSTOM_REAL*meshX%dxc(i  ))-dbxdy/(2._CUSTOM_REAL*meshY%dxc(j+1))
...
!
            ...
            bz3=bz(i  ,j  ,k)  +bdipole_z(i  ,j  ,k)
            bz4=bz(i+1,j  ,k)  +bdipole_z(i+1,j  ,k)
            bxav=.25_CUSTOM_REAL*(bx1+bx2+bx3+bx4)
            byav=.25_CUSTOM_REAL*(by1+by2+by3+by4)
            bzav=.25_CUSTOM_REAL*(bz1+bz2+bz3+bz4)
            xj = curlbx_scalar
            yj = curlby_scalar
            zj = curlbz_scalar
	    bxx = bxav
	    byy = byav
	    bzz = bzav
            xtmp1m = sqrt(bxx**2_CUSTOM_REAL + byy**2_CUSTOM_REAL + bzz**2_CUSTOM_REAL)
...
!
       call XREALBCC_PACK_E_2D(fox,foy,foz,one,NX,NY,NZ) !LAURA
...
!
      ...
      epsilon=1.E-05_CUSTOM_FLUX	!LAURA
      outer_radius=dipole_sphere_radius+min(hx,hy)
      eps2=1.E-25_CUSTOM_FLUX	!LAURA
...
!               Nonuniform mesh - using MESH_UNMAP
                rxb=dtxi*MESH_UNMAP(meshX,x(l))+1.50000000000E+00_CUSTOM_FLUX    !LAURA
                ryb=dtyi*MESH_UNMAP(meshY,y(l))+1.50000000000E+00_CUSTOM_FLUX    !LAURA
                rzb=dtzi*MESH_UNMAP(meshZ,z(l))+1.50000000000E+00_CUSTOM_FLUX    !LAURA
                ixb=rxb
                iyb=ryb
                izb=rzb
...
!               Nonuniform mesh - using MESH_UNMAP
                rxe=dtxi*MESH_UNMAP(meshX,x(l))+1.50000000000E+00_CUSTOM_FLUX    !LAURA
                rye=dtyi*MESH_UNMAP(meshY,y(l))+1.50000000000E+00_CUSTOM_FLUX    !LAURA
                rze=dtzi*MESH_UNMAP(meshZ,z(l))+1.50000000000E+00_CUSTOM_FLUX    !LAURA
                ixe=rxe
                iye=rye
                ize=rze
                IZE=1
...
!             Nonuniform mesh - using MESH_UNMAP
              rxe=dtxi*MESH_UNMAP(meshX,x(np))+1.50000000000E+00_CUSTOM_FLUX   !LAURA
              rye=dtyi*MESH_UNMAP(meshY,y(np))+1.50000000000E+00_CUSTOM_FLUX   !LAURA
              rze=dtzi*MESH_UNMAP(meshZ,z(np))+1.50000000000E+00_CUSTOM_FLUX   !LAURA
              ixe=rxe
              iye=rye
              ize=rze
              IZE=1
...
!               Nonuniform mesh - using MESH_UNMAP
                rxe=dtxi*MESH_UNMAP(meshX,x(nprecv))+1.50000000000E+00_CUSTOM_FLUX    !LAURA
                rye=dtyi*MESH_UNMAP(meshY,y(nprecv))+1.50000000000E+00_CUSTOM_FLUX    !LAURA
                rze=dtzi*MESH_UNMAP(meshZ,z(nprecv))+1.50000000000E+00_CUSTOM_FLUX    !LAURA
                ixe=rxe 
                iye=rye 
                ize=rze 
                IZE=1
...
!              Nonuniform mesh - using MESH_UNMAP
                rx=dtxi*MESH_UNMAP(meshX,x(l))+1.50000000000E+00_CUSTOM_FLUX  !LAURA
                ry=dtyi*MESH_UNMAP(meshY,y(l))+1.50000000000E+00_CUSTOM_FLUX  !LAURA
                rz=dtzi*MESH_UNMAP(meshZ,z(l))+1.50000000000E+00_CUSTOM_FLUX  !LAURA
                ix=rx 
                iy=ry 
                iz=rz 
                fx=rx-ix 
